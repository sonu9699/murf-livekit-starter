...
async def main():
    ...
    if missing_common:
        print(f"Error: Missing required LiveKit environment variables in .env.local: {', '.join(missing_common)}")
        sys.exit(1)
    ...
    if args.provider == "linphone":
        if not os.getenv("LINPHONE_USERNAME"):
            ...
            print("Please add 'LINPHONE_USERNAME=your_username' to your .env.local file first.")
            sys.exit(1)
        ...
        if missing_twilio:
            print(f"Error: Missing required Twilio environment variables in .env.local: {', '.join(missing_twilio)}")
            sys.exit(1)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print(f"SUCCESS! {args.provider.capitalize()} SIP Outbound Trunk created successfully.")
