      ...
      "embeds": [
        {
          ...
          "fields": [
            ...
            {"name": "Preferred Follow-up", "value": followup_method, "inline": True}
          ],
...
async def save_escalation(
    ...
    with _connect(path) as conn:
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'open', ?)
            """,
            (ref_id, caller_name, language, symptoms, urgency, followup_method, summary, created_at)
        )
        conn.commit()
    ...
    if webhook_url:
        await send_discord_webhook(webhook_url, ref_id, caller_name, language, symptoms, urgency, followup_method, summary, created_at)
